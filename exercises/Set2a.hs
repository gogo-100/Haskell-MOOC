...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = take i xs ++ [x] ++ takeFinal (len-i-1) xs
    where len = length xs
...
isPalindrome str 
    | len == 0  = True
    | len == 1  = True
    | otherwise = (str!!0 == str!!(len-1)) && (isPalindrome (substring 1 (len-1) str))
    where len = length str
-- 答案里这条是 str == reverse str
...
palindromify s
    | isPalindrome s     = s
    | otherwise          = palindromify(substring 1 (len-1) s)
    where len = length s
